...
// GLCodeQualityConversionForModule converts the sarif reports into the GitLab code quality format (gl-codequality).
func GLCodeQualityConversionForModule(sdk cidsdk.SDKClient, ctx cidsdk.ModuleActionData, sarifFile string) error {
	// gitlab conversion
	if ctx.Env["NCI_REPOSITORY_HOST_TYPE"] == "gitlab" {
		// code-quality report
		codeQualityFile := cidsdk.JoinPath(ctx.Config.TempDir, "gl-code-quality-report.json")
